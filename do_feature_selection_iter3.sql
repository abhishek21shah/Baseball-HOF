...
CREATE TABLE `FieldingF` (
  ...
  `FieldPercent` FLOAT (11) DEFAULT 0,
  PRIMARY KEY (playerID)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
SELECT t.playerID, SUM(t.G),  IF(SUM(t.AB)>0, (SUM(t.H)/SUM(t.AB)), 0), AVG(t.HR)
FROM Batting t
Group by t.playerID;

INSERT INTO FieldingF
SELECT t.playerID, SUM(t.G), AVG(if((t.PO + t.A + t.E)>0, (t.PO + t.A)/(t.PO + t.A + t.E), 0)) 
FROM Fielding t
Group by t.playerID;

INSERT INTO PitchingF 
SELECT t.playerID, SUM(t.W), SUM(t.L), AVG(if(t.BB>0, t.SO/t.BB, 0))
FROM Pitching t
Group by t.playerID;
...
SELECT m.playerID, daysActive, BatG, BatBA, BatHR, FieldG, FieldPercent, PitchW, PitchL, PitchKBB, allStarCount, Cy_Young_Award, Gold_Glove_Award, Silver_Slugger, MVP, avgSalary, inducted
FROM MasterF m 
...
FieldPercent = COALESCE(FieldPercent, 0),
PitchW = COALESCE(PitchW, 0),
PitchL = COALESCE(PitchL, 0),
PitchKBB = COALESCE(PitchKBB, 0),
